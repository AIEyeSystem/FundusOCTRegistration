...
def warp_and_combine(
    ...
    print(src_transformed.shape,dst_image.shape)
    dst_image = resize(dst_image, dst_scale).transpose(1,0)
...
def run(
    ...
    _, _, agf_seg = preprocess_agf(agf_image,sigma=2.0)
    _, _, octa_seg = preprocess_octa(octa_image,sigma=1.5, hyst_th_percent=info_th)


    cv2.imwrite('afg.png',agf_seg)
    cv2.imwrite('octa.png',octa_seg)
